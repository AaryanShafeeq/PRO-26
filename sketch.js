...
function handlePlayerArrowCollision() {
  ...
  for (let i = 0; i < playerArrows.length; i++) {
    let baseCollision = Matter.SAT.collides(
      playerArrows[i].body,
      computerBase.body
    );

    let archerCollision = Matter.SAT.collides(
      playerArrows[i].body,
      computerArcher.body
    );

    let computerCollision = Matter.SAT.collides(
      playerArrows[i].body,
      computerArcher.body
    );
...
function handleComputerArrowCollision() {
  ...
  for (let i = 0; i < computerArrows.length; i++) {
    ...
    let playerCollision = Matter.SAT.collides(
      computerArrows[i].body,
      playerArcher.body
    );
